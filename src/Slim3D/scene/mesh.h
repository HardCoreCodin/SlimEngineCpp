...
struct Mesh {
    ...
    void draw(const Viewport &viewport, const Transform &xform, bool draw_normals, const vec3 &color = Color(White), f32 opacity = 1.0f, u8 line_width = 1) {
        ...
        vec3 pos;
        Edge edge;
        for (u32 i = 0; i < edge_count; i++, edge_vertex_indices++) {
            edge.from = cam.internPos(xform.externPos(vertex_positions[edge_vertex_indices->from]));
            edge.to   = cam.internPos(xform.externPos(vertex_positions[edge_vertex_indices->to]));
            edge.draw(viewport, color, opacity, line_width);
        }

        if (draw_normals && normals_count && vertex_normals && vertex_normal_indices) {
            for (u32 t = 0; t < triangle_count; t++, vertex_normal_indices++, vertex_position_indices++) {
                for (u8 i = 0; i < 3; i++) {
                    pos     = vertex_positions[vertex_position_indices->ids[i]];
                    edge.to = vertex_normals[    vertex_normal_indices->ids[i]] * 0.1f + pos;
